...
r.url().includes('/api/incident') && r.status() === 200 && !r.url().includes('comments') && !r.url().includes('contact')
);
...
```
// 3a – assessment / property (the "inner" JSON)
const assessResp = await page.waitForResponse(r =>
  r.url().includes(`/api/incident/${id}`) && !r.url().includes('comments') && !r.url().includes('contact')
);
...
rows.push({
  ...
  phone: phoneMatch[0] || (contact.contactNotes?.[0]?.contact?.match(/\\d{3}[-.\\s]?\\d{3}[-.\\s]?\\d{4}/)?.[0]) || null,
